 ...
 * 有改动：
 * - 增加了setupIcon方法，用以包裹输入框和附加一个icon
 * by zh25745
 */
"use strict";
...
function Flatpickr(element, config) {
	...
	function init() {
		...
		self.parentNode = element.parentNode;
		self.nextSibling = element.nextSibling;
	...
	function buildWeekdays() {
		...
		// var weekdaysHtml = weekdays.join("</span><span class=flatpickr-weekday>");

		var weekdaysHtml = "";
		for (var i = 1; i <= weekdays.length; i++) {
			if (i == 1 || i == weekdays.length) {
				weekdaysHtml += "<span class='flatpickr-weekday rest'>" + weekdays[i-1] + "</span>";
			} else {
				weekdaysHtml += "<span class=flatpickr-weekday>" + weekdays[i-1] + "</span>";
			}
		}

		self.weekdayContainer.innerHTML =  weekdaysHtml ;
	...
	function setupIcon() {
		...
		className = className.replace('flatpickr-input','').replace(/(active|flatpickr|J\-\w+)/gi,'');
		var fa = document.createElement('div');
		fa.className = "flatpickr-wrapper " + className;
		fa.appendChild(self.element);
		var icon = document.createElement('span');
		icon.className = "flatpickr-icon";
		fa.appendChild(icon);
		// self.parentNode.removeChild(self.element);
		self.parentNode.insertBefore(fa, self.nextSibling);
	}
